...
def config_for(name: str, url: str, steps: list[dict], size: tuple[int, int],
               pause: int, scheme: str, serve: str | None) -> dict:
    return {
        "url": url.rstrip("/"),
        "steps": steps,
...
def build() -> str:
    """The runner is fixed; every demo's settings live in its .web.json."""
    return (
...
def main() -> int:
    ...
    config = config_for(args.name, args.url, steps,
    ...
    settings = out.with_suffix(".json")
    settings.write_text(json.dumps(config, indent=2) + "\n", encoding="utf-8")
    print(f"wrote {out} and {settings} ({len(steps)} steps)")
    return 0
